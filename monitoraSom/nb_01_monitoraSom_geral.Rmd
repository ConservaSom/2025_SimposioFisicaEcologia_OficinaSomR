...
df_templates <- head(df_templates, 2)
...
cowplot::plot_grid(
    templates_plots[[2]], templates_plots[[1]],
...
plot_scores(
    ...
    dyn_range = c(-96, -48), color_scale = "inferno", zoom_time = c(0, 30)
)
```
...
plot_scores(
    ...
    dyn_range = c(-96, -48), color_scale = "inferno", zoom_time = c(20, 25)
)
```
...
Agora vamos desligar o buffer e reter somente as detecções com scores acima de 0.2. Podemos ver que nesse caso o limiar de 0.2 ainda não foi suficiente para eliminar todas as detecções que conseguimos ver que são claramente falsas.
```{r}
plot_scores(
    df_scores_i = df_scores[7, ], ovlp = 70, wl = 1024,
    dyn_range = c(-96, -48), color_scale = "inferno",
    buffer_size = 0, min_score = 0.2
)
```

Podemos subir o limiar para 0.287 e ver que agora as detecções falsas foram eliminadas.
```{r}
plot_scores(
    df_scores_i = df_scores[7, ], ovlp = 70, wl = 1024,
    dyn_range = c(-96, -48), color_scale = "inferno",
...
Mas... Se usarmos o mesmo limiar para outro match, podemos verificar que ele foi suficiente para eliminar todas as detecções falsas. Note também que apesar disso ainda temos muitos picos sobrepostos, já que o filtro de buffer foi desligado.
```{r}
plot_scores(
    df_scores_i = df_scores[9, ], ovlp = 70, wl = 1024,
    dyn_range = c(-96, -48), color_scale = "inferno",
    buffer_size = 0, min_score = 0.287
)
```
...
```{r}
df_detecs_aposteriori <- read.csv(file = "./detections/df_detecs_aposteriori.csv")
...
ls_val_apriori <- diagnostic_validations(
    df_validated = df_validated, pos_prob = 0.8, val_a_priori = TRUE
)
