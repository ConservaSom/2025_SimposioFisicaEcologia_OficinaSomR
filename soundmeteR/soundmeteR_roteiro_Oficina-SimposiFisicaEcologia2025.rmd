...
matrix(
  ...
  kableExtra::footnote(
    general_title = "",
    "**Nota**: Todas as funções permitem a escolha de diferentes curvas de ponderação de frequências (como curvas A, B, C e Z)",
...
## Carregando os pacotes e o arquivos de exemplo

Carregando os pacotoes necessários:
```{r eval = FALSE}
...
Carregando o exemplo interno do pacote e visualizando o espectrograma. O exemplo é um canto de um _Thamnophilus stictocephalus_ em uma gravação calibrada.
```{r eval = FALSE}
...
## Aplicações do _soundmeteR_ em gravações não calibradas
...
Abaixo realizamos a leitura das planilhas com as detecções utilizando uma função do pacote _monitoraSom_.
```{r eval = FALSE}
detections <- monitoraSom::fetch_rois(
  rois_path = "./final_detecs",
) %>%
  print()
```

A partir da planilha com as detecções, vamos extrair medidas de pressão sonora de cada detecção com a função `soundmeter`. 
```{r eval = FALSE}
spl <- soundmeter(
  files = detections$soundscape_path,
  channel = "left",
  from = detections$roi_start,
  to = detections$roi_end,
...
Vendo os valores de Lmax junto com os sonogramas
```{r eval = FALSE}
# spl <- spl %>%
#   arrange(LAmax) %>%
#   print()

layout(matrix(1:20, nrow = 2, byrow = TRUE))

for (i in seq_len(nrow(spl))) {
  detec <- readWave(
    detections$soundscape_path[i],
    from = detections$roi_start[i],
    to = detections$roi_end[i],
    units = "seconds"
  )
