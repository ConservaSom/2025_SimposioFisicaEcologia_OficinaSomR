...
```{r eval = FALSE}
library(soundmeteR) # carregar o pacote soundmeteR
library(seewave) # caregando seewave para visualização do sonogramas
library(tuneR) # carregando tuneR para manipulação de arquivos Wave

data(tham) #aquivo de exemplo do pacote soundmeteR (Thamnophilus stictocephalus)
...
### Gravações não calibradas
- leqbands em dBFS

- soundmeter em dBFS (ruído de fundo)

### Gravações com calibração
- Calibração com arquivos separado
  - leqbands (pensar em separar a função de calibração)
  - soundmeter

- Calibração com sinal de referência no mesmo arquivo
  - leqbands_cal
  - soundmeter

### Arquivos Wave não carregados no ambiente R
...
- Artigo do _Basileuterus culicivorus_
- Contextualizar
   - PAM
   - Calibração
...
## Extração da medida de interesse
